...
async function collectAttackPathChecks(graph, io, findings, summary, ctx = {}) {
  ...
  // Self-service sign-up, group creation and directory read (D6).
  if (authz) {
    const selfSignup = authz.allowEmailVerifiedUsersToJoinOrganization === true;
    checklist.push(
      checklistRow(
        "AP.Guest.SelfServiceSignup",
        "Email-verified users must not be able to join the tenant on their own",
        selfSignup ? CHECK.FAIL : CHECK.PASS,
        "Medium",
        `allowEmailVerifiedUsersToJoinOrganization=${authz.allowEmailVerifiedUsersToJoinOrganization}`,
        "Anyone with a verified e-mail on a tenant domain gets a member account without an admin"
      )
    );
    if (selfSignup) {
      push(
        ...
        "Medium",
        "Guests",
        "Self-service sign-up is on (allowEmailVerifiedUsersToJoinOrganization=true) — unmanaged accounts can join the tenant without an invitation"
      );
    }
    const createGroups = dup.allowedToCreateSecurityGroups !== false;
    checklist.push(
      checklistRow(
        "AP.Users.CreateSecurityGroups",
        "Non-admins should not create security groups",
        createGroups ? CHECK.FAIL : CHECK.PASS,
        "Low",
        `allowedToCreateSecurityGroups=${dup.allowedToCreateSecurityGroups}`,
        "Attacker-owned groups feed app role assignments, sharing and CA include scopes"
      )
    );
    if (createGroups) {
      push(
        ...
        "Low",
        "Users",
        "Any user can create security groups (allowedToCreateSecurityGroups) — restrict to admins / owners of a naming policy"
      );
    }
    if (dup.allowedToReadOtherUsers === false) {
      push(
        findings,
        "Info",
        "Users",
        "allowedToReadOtherUsers=false — member enumeration is restricted (hardened; verify apps that rely on people picker)"
      );
    }
  }
  ...
  // Until the legacy MFA / SSPR policies are migrated, the Authentication
  // methods policy is not the only source of truth: SMS or voice can still be
  // enabled per-user in the legacy MFA portal while showing "disabled" here,
  // and vice-versa. Every verdict above is then Partial.
  const migration = String(
    (authMethods && authMethods.policyMigrationState) || facts.policyMigrationState || ""
  );
  if (!missing("authMethods") && migration && !/^migrationComplete$/i.test(migration)) {
    for (let i = authMethodsMark.checklist; i < checklist.length; i++) {
      const row = checklist[i];
      if (!/^AP\.MFA\./.test(row.CheckId) || row.Status === CHECK.INFO) continue;
      row.Status = CHECK.PARTIAL;
      row.Confidence = "Medium";
      row.Rationale = `policyMigrationState=${migration}: legacy MFA/SSPR policies still apply`;
      row.Evidence = `${row.Evidence} — legacy per-user MFA / SSPR policy still authoritative (migration ${migration})`;
    }
    push(
      findings,
      "Medium",
      "WeakMFA",
      `Authentication methods policy migration is "${migration}" — the legacy per-user MFA and SSPR policies still control which methods are usable. Method verdicts are Partial until the migration is completed (Entra admin center → Authentication methods → Manage migration).`
    );
    checklist.push(
      checklistRow(
        "AP.MFA.Migration",
        "Authentication methods policy migration completed",
        CHECK.FAIL,
        "Medium",
        `policyMigrationState=${migration}`,
        "Two policy surfaces → SMS / voice may be usable despite the modern policy",
        { confidence: "High", rationale: "Legacy MFA / SSPR policies remain in effect" }
      )
    );
  } else if (!missing("authMethods") && /^migrationComplete$/i.test(migration)) {
    checklist.push(
      checklistRow(
        "AP.MFA.Migration",
        "Authentication methods policy migration completed",
        ...
        "Single authoritative method policy"
      )
    );
  }
  ...
  //
  // Every verdict below is computed on the *effective scope* of each policy
  // (lib/caScope.js): a policy only counts as coverage when it is enforced,
  // targets all users and all apps (or the roles the control is about) and is
  // not hollowed out by exclusions. Anything else is Partial, never Pass.
  console.log("  · Conditional Access attack-path coverage");
  const caMark = mark();
  const caCoverage = [];

  // 3a) Exclusion groups first — their size feeds the "broad exclusions" test.
  ...
  const exclGroupRows = [];
  const groupMeta = {};
  async function countOrNull(url) {
    try {
      const v = await graph.get(url);
      const n = Number(typeof v === "object" && v !== null ? v.value ?? v["@odata.count"] : v);
  ...
  for (const gid of excludeGroupIds) {
    ...
    if (!g) continue;
    const memberCount = await countOrNull(`${graph.GRAPH}/groups/${gid}/members/$count`);
    const ownerCount = await countOrNull(`${graph.GRAPH}/groups/${gid}/owners/$count`);
    const dynamic = !!(g.membershipRule || (g.groupTypes || []).includes("DynamicMembership"));
    const protected_ = g.isAssignableToRole === true;
    const excludingPolicies = enforced.filter((p) =>
      (usersCond(p).excludeGroups || []).includes(gid)
    );
    const usedBy = excludingPolicies.map((p) => p.displayName);
    // Complementary design: "MFA for internals" excludes the guests group and
    // "MFA for guests" excludes the internals group. The excluded group is the
    // *include target* of another enforced policy with the same kind of grant,
    // so its members are covered elsewhere — that is scoping, not a bypass.
    const grantKind = (p) => (isBlock(p) ? "block" : requiresMfa(p) ? "mfa" : "other");
    const coveredBy = enforced.filter(
      (p) =>
        (usersCond(p).includeGroups || []).includes(gid) &&
        excludingPolicies.some((x) => grantKind(x) === grantKind(p) && grantKind(p) !== "other")
    );
    const complementary = coveredBy.length > 0;
    groupMeta[gid] = {
      displayName: g.displayName,
      memberCount,
      ownerCount,
      dynamic,
      isAssignableToRole: protected_,
      complementary,
    };
    exclGroupRows.push({
      GroupId: g.id,
      ...
      Dynamic: dynamic,
      MembershipRule: dynamic ? String(g.membershipRule || "").slice(0, 200) : "",
      MemberCount: memberCount,
      OwnerCount: ownerCount,
      UsedByPolicies: usedBy.length,
      Policies: usedBy.slice(0, 8).join(" | "),
      Complementary: complementary,
      CoveredByPolicies: coveredBy.map((p) => p.displayName).slice(0, 4).join(" | "),
      Hardened: complementary || (protected_ && !dynamic),
      Risk: complementary
        ? `Complementary — group is the include target of "${coveredBy[0].displayName}"; members are covered there, not bypassing`
        : dynamic
          ? "Weak — dynamic membership: an attribute change (title, department, …) adds the account and bypasses CA"
          : protected_
            ? ownerCount
              ? `Review — role-assignable but ${ownerCount} owner(s) can still add members`
              : "OK — role-assignable (only GA / PRA can change membership)"
            : "Weak — any User Administrator / group owner can add members → CA bypass",
    });
  }
  ...
  if (weakExclGroups.length) {
    ...
    const members = weakExclGroups.reduce(
      (n, r) => n + (Number.isFinite(Number(r.MemberCount)) ? Number(r.MemberCount) : 0),
      0
    );
    push(
      findings,
      "High",
      "CA",
      `${weakExclGroups.length}/${exclGroupRows.length} CA exclusion groups are not hardened (not role-assignable or dynamic)` +
        (members ? ` — ${members} member(s) currently bypass the policies that exclude them` : "") +
        ": " +
        shown +
  ...
  if (excludeGroupIds.size === 0) {
    checklist.push(
      checklistRow(
        "AP.CA.ExclGroups",
        "CA exclusion groups should be role-assignable and static",
        CHECK.INFO,
        "High",
    ...
    const weak = weakExclGroups.length;
    const bigOk = exclGroupRows.filter(
      (r) => r.Hardened && Number(r.MemberCount) >= 25
    );
    checklist.push(
      checklistRow(
        "AP.CA.ExclGroups",
        "CA exclusion groups should be role-assignable and static",
        weak ? CHECK.FAIL : bigOk.length ? CHECK.PARTIAL : CHECK.PASS,
        "High",
        `${weak}/${exclGroupRows.length} exclusion groups weak` +
          (bigOk.length
            ? `; ${bigOk.length} hardened group(s) still hold ≥25 members (${bigOk.map((r) => `${r.DisplayName}=${r.MemberCount}`).join(", ")})`
            : "") +
          ` — total excluded members: ${exclGroupRows.reduce((n, r) => n + (Number(r.MemberCount) || 0), 0)}`,
        "CA bypass via group membership"
      )
    );
  }
  ...
  const scopes = policies.map((p) =>
    effectiveScope(p, {
      ...
      authStrengths: ctx.authStrengths || [],
      groupMeta,
    })
  );
  const enforcedScopes = scopes.filter((s) => s.enforced);
  const verdict = (pred, opts) => coverageVerdict(scopes, pred, opts);
  const statusOf = (v) =>
    v.status === "full" ? CHECK.PASS : v.status === "partial" ? CHECK.PARTIAL : CHECK.FAIL;
  const coveredOf = (v) => (v.status === "full" ? true : v.status === "partial" ? "partial" : false);
  function coverageRow(control, v, severity, why) {
    caCoverage.push({
      Control: control,
      Covered: coveredOf(v),
      Policies: v.names.join(" | "),
      Severity: severity,
      Why: why,
      Evidence: v.reasons.join("; "),
    });
  }
  const namesOf = (arr) => arr.map((s) => s.name).join(" | ");
  /** A risk-conditioned policy only fires on risk; it is not everyday coverage. */
  const notRiskGated = (s) => !s.signInRisk.length && !s.userRisk.length;
  const partialEvidence = (v, fallback) => {
    if (v.status !== "partial") return fallback;
    if (!v.partial.length && v.reportOnly.length) {
      return `Report-only only — not enforced: ${namesOf(v.reportOnly)}`;
    }
    return (
      `Only scoped / carved-out coverage: ${v.reasons.join("; ") || v.names.join(" | ")}` +
      (v.reportOnly.length ? ` (also report-only: ${namesOf(v.reportOnly)})` : "")
    );
  };

  // Device code / auth transfer — block (or at least MFA) for everyone.
  const deviceCodeV = verdict(
    (s) =>
      (s.authFlows.includes("devicecodeflow") || s.authFlows.includes("authenticationtransfer")) &&
      (s.grants.isBlock || s.grants.requiresMfa)
  );
  coverageRow(
    "Block or MFA device code / auth transfer",
    deviceCodeV,
    "High",
    "Device code phishing / Graph CLI abuse (AitM-adjacent)"
  );
  checklist.push(
    checklistRow(
      "AP.CA.DeviceCode",
      "CA must block device code / authentication transfer for all users",
      statusOf(deviceCodeV),
      "High",
      deviceCodeV.status === "none"
        ? "No enforced policy with authenticationFlows.transferMethods"
        : partialEvidence(deviceCodeV, namesOf(deviceCodeV.full)),
      "Device-code phishing yields a primary refresh token without a password"
    )
  );
  if (deviceCodeV.status !== "full") {
    push(
      findings,
      deviceCodeV.status === "none" ? "High" : "Medium",
      "CA",
      deviceCodeV.status === "none"
        ? "No enforced CA blocks device code / authentication transfer for all users"
        : `Device-code CA is scoped or carved out — not population-wide: ${deviceCodeV.reasons.join("; ")}`
    );
  }

  // Legacy auth block — all users, all apps (or Office 365).
  const legacyV = verdict((s) => targetsLegacyClients(s) && s.grants.isBlock, {
    population: (s) => s.enforced && s.allUsers && (s.allApps || s.office365) && !s.broadExclusions,
  });
  coverageRow(
    "Block legacy authentication (EAS / Other clients)",
    legacyV,
    "High",
    "Password spray / basic auth bypasses modern MFA"
  );
  checklist.push(
    checklistRow(
      "AP.CA.LegacyBlock",
      ...
      legacyV.status === "none"
        ...
        : partialEvidence(legacyV, namesOf(legacyV.full)),
      "Password spray without MFA"
    )
  );
  if (legacyV.status === "none") {
    push(
      findings,
      "High",
      "CA",
  ...
  } else if (legacyV.status === "partial") {
    push(
      findings,
      "High",
      "CA",
      !legacyV.partial.length && legacyV.reportOnly.length
        ? `Legacy-auth block exists only in report-only mode (${namesOf(legacyV.reportOnly)}) — nothing is enforced; basic auth / password spray path open until switched On`
        : `Legacy-auth block is not population-wide: ${legacyV.reasons.join("; ")} — accounts outside the scope can still use basic auth`
    );
  }

  // Azure management / admin portals MFA for admins.
  const adminPop = (s) => s.enforced && !s.broadExclusions && (s.allUsers || s.includedRoles.length > 0);
  const azureV = verdict(
    (s) =>
      includesApp(s, APP.AZURE_MANAGEMENT) &&
      s.grants.requiresMfa &&
      notRiskGated(s) &&
      targetsAdmins(s),
    { population: adminPop }
  );
  const portalsV = verdict(
    (s) =>
      includesApp(s, APP.MICROSOFT_ADMIN_PORTALS) &&
      s.grants.requiresMfa &&
      notRiskGated(s) &&
      targetsAdmins(s),
    { population: adminPop }
  ...
  const azureCombined = {
    status:
      azureV.status === "full"
        ? "full"
        : portalsV.status === "full" || azureV.status === "partial" || portalsV.status === "partial"
          ? "partial"
          : "none",
    names: [...new Set([...azureV.names, ...portalsV.names])],
    reasons: [...azureV.reasons, ...portalsV.reasons],
    full: azureV.full,
    partial: [...azureV.partial, ...portalsV.partial],
    reportOnly: [...azureV.reportOnly, ...portalsV.reportOnly],
  };
  coverageRow(
    "MFA for Azure management / Admin portals",
    azureCombined,
    "High",
    "Stolen password/session → Azure + often Entra control plane"
  );
  if (azureCombined.status === "none") {
    checklist.push(
      checklistRow(
        "AP.CA.AzureMgmt",
  ...
  } else if (azureCombined.status === "partial") {
    // Admin portals fully covered but the ARM API only by scoped / report-only
    // policies (or not at all) is the classic "browser yes, CLI no" gap.
    const portalsOnly = portalsV.status === "full" && azureV.status !== "full";
    checklist.push(
      checklistRow(
        "AP.CA.AzureMgmt",
        "CA must require MFA for Azure management / admin portals",
        ...
        "High",
        portalsOnly
          ? `Admin portals only: ${namesOf(portalsV.full)} — Azure Service Management API (ARM / CLI / PowerShell) not covered by an enforced admin policy` +
            (azureV.status === "partial" ? ` (${partialEvidence(azureV, "")})` : "")
          : partialEvidence(azureCombined, azureCombined.names.join(" | ")),
    ...
    push(
      ...
      portalsOnly
        ...
        : `Azure-management MFA CA is scoped or carved out: ${azureCombined.reasons.join("; ")}`
    );
  } else {
    checklist.push(
      checklistRow(
  ...
  // Sign-in risk / user risk (Identity Protection — P2).
  const signInRiskV = verdict(
    (s) => s.signInRisk.length > 0 && (s.grants.requiresMfa || s.grants.isBlock)
  );
  const userRiskV = verdict(
    (s) =>
      s.userRisk.length > 0 &&
      (s.grants.requiresMfa || s.grants.isBlock || s.grants.passwordChange)
  );
  coverageRow(
    "Sign-in risk → MFA or block",
    signInRiskV,
    "High",
    "Stops impossible travel / unfamiliar token use mid-attack"
  );
  coverageRow(
    "User risk high → block or password change",
    userRiskV,
    "High",
    "Identity Protection at-risk users keep accessing if no user-risk CA"
  );
  checklist.push(
    checklistRow(
      "AP.CA.SignInRisk",
      "CA for sign-in risk",
      statusOf(signInRiskV),
      "High",
      signInRiskV.status === "none" ? "none" : partialEvidence(signInRiskV, namesOf(signInRiskV.full)),
      "Risky sign-ins",
      { licence: "P2" }
    )
  );
  if (signInRiskV.status === "none") {
    push(findings, "High", "CA", "No enforced CA for sign-in risk (MFA/block)");
  } else if (signInRiskV.status === "partial") {
    push(findings, "Medium", "CA", `Sign-in-risk CA is not population-wide: ${signInRiskV.reasons.join("; ")}`);
  }
  checklist.push(
    checklistRow(
      "AP.CA.UserRisk",
      ...
      statusOf(userRiskV),
      "High",
      userRiskV.status === "none" ? "none" : partialEvidence(userRiskV, namesOf(userRiskV.full)),
      "Compromised accounts stay active",
      { licence: "P2" }
    )
  );
  if (userRiskV.status === "none") {
    push(
      findings,
      "High",
      "CA",
  ...
  } else if (userRiskV.status === "partial") {
    push(findings, "Medium", "CA", `User-risk CA is not population-wide: ${userRiskV.reasons.join("; ")}`);
  }

  // MFA for guests — explicit guest targeting, all apps, no carve-outs.
  const guestV = verdict(
    (s) =>
      s.includesGuests && s.grants.requiresMfa && notRiskGated(s) && (s.allApps || s.office365),
    { population: (s) => s.enforced && !s.broadExclusions }
  );
  // All-users MFA that does not exclude guests also covers them.
  const allUsersMfaCoversGuests = enforcedScopes.filter(
    (s) => coversPopulation(s) && s.grants.requiresMfa && !s.excludesGuests && !s.signInRisk.length && !s.userRisk.length
  );
  const guestCovered = guestV.status === "full" || allUsersMfaCoversGuests.length > 0;
  coverageRow(
    "MFA for guest / external users",
    guestV.status === "full"
      ? guestV
      : allUsersMfaCoversGuests.length
        ? { status: "full", names: allUsersMfaCoversGuests.map((s) => s.name), reasons: ["via All-users MFA (guests not excluded)"] }
        : guestV,
    "High",
    "Invited guest with password-only access to SPO/Teams"
  );
  checklist.push(
    checklistRow(
      "AP.CA.GuestMfa",
      "CA must require MFA for guests",
      guestV.status === "full" ? CHECK.PASS : guestCovered ? CHECK.PARTIAL : statusOf(guestV),
      "High",
      guestV.status === "full"
        ? namesOf(guestV.full)
        : guestCovered
          ? `Covered only via All-users MFA (${namesOf(allUsersMfaCoversGuests)}) — no guest-specific policy`
          : guestV.status === "partial"
            ? partialEvidence(guestV, "")
            : "No guest-targeted MFA CA",
      "Guest foothold after invite"
    )
  );
  if (guestV.status !== "full") {
    push(
      findings,
      guestCovered ? "Medium" : "High",
      "CA",
      guestCovered
        ? "No guest-specific MFA CA — relying on All-users policies (verify guest exclusions and B2B trust settings)"
        : guestV.status === "partial"
          ? `Guest MFA CA is scoped or carved out: ${guestV.reasons.join("; ")}`
          : "No enforced MFA CA targeting guests/external users"
    );
  }

  // Population MFA (D1) — the baseline every other control assumes.
  const allUsersMfaV = verdict(
    (s) =>
      s.grants.requiresMfa &&
      !s.signInRisk.length &&
      !s.userRisk.length &&
  ...
  coverageRow(
    ...
    allUsersMfaV,
    "High",
    "Password-only sign-in anywhere in the population"
  );
  checklist.push(
    checklistRow(
      "AP.CA.AllUsersMfa",
      "CA must require MFA for all users on all cloud apps",
      statusOf(allUsersMfaV),
      "High",
      allUsersMfaV.status === "none"
        ? "No enforced All-users / All-apps MFA policy (risk-based and user-action policies do not count)"
        : partialEvidence(allUsersMfaV, namesOf(allUsersMfaV.full)),
      "Password spray / stuffing succeeds against any account outside MFA scope"
    )
  );
  if (allUsersMfaV.status === "none") {
    push(
      findings,
      "High",
      "CA",
      "No enforced Conditional Access requires MFA for all users on all cloud apps"
    );
  } else if (allUsersMfaV.status === "partial") {
    push(
      findings,
      "High",
      "CA",
      `Population MFA is scoped or carved out — ${allUsersMfaV.reasons.join("; ")} — accounts outside it sign in with a password only`
    );
  }

  // Phishing-resistant MFA for Global Administrators (D1).
  const adminPrPop = (s) =>
    s.enforced &&
    !s.excludedGaRoles.length &&
    (s.allApps || s.office365 || includesApp(s, APP.AZURE_MANAGEMENT) || includesApp(s, APP.MICROSOFT_ADMIN_PORTALS));
  const adminPrV = verdict(
    (s) => targetsGlobalAdmin(s) && s.grants.phishingResistant && notRiskGated(s),
    { population: adminPrPop }
  );
  const adminMfaV = verdict(
    (s) => targetsGlobalAdmin(s) && s.grants.requiresMfa && notRiskGated(s),
    { population: adminPrPop }
  );
  coverageRow(
    ...
    adminPrV,
    "High",
    "AitM / token-replay against admins"
  );
  checklist.push(
    checklistRow(
      "AP.CA.AdminPhishingResistant",
      "Global Administrators must use phishing-resistant MFA (authentication strength)",
      adminPrV.status === "full"
        ? CHECK.PASS
        : adminMfaV.status === "full" || adminPrV.status === "partial"
          ? CHECK.PARTIAL
          : CHECK.FAIL,
      "High",
      adminPrV.status === "full"
        ? namesOf(adminPrV.full)
        : adminPrV.status === "partial"
          ? partialEvidence(adminPrV, "")
          : adminMfaV.status === "full"
            ? `Admins get MFA (${namesOf(adminMfaV.full)}) but no phishing-resistant authentication strength`
            : "No enforced policy targets Global Administrator with MFA or an authentication strength",
      "Phishable MFA on the accounts that can rewrite the tenant"
    )
  );
  if (adminPrV.status !== "full") {
    push(
      findings,
      adminMfaV.status === "full" ? "Medium" : "High",
      "CA",
      adminMfaV.status === "full"
        ? "Global Administrators are MFA-protected but not required to use phishing-resistant methods (FIDO2 / passkey / WHfB / CBA)"
        : "No enforced CA requires MFA or an authentication strength for Global Administrators"
    );
  }

  // Exclusions of GA / PRA roles from population policies.
  const gaExcluded = enforcedScopes.filter((s) => s.allUsers && s.excludedGaRoles.length);
  checklist.push(
    checklistRow(
      "AP.CA.ExclRoles",
      "All-users policies must not exclude Global / Privileged Role Administrators",
      gaExcluded.length ? CHECK.FAIL : CHECK.PASS,
      "High",
      gaExcluded.length
        ? gaExcluded.map((s) => `${s.name} excludes ${s.excludedGaRoles.join(", ")}`).join(" | ")
        : "No enforced All-users policy excludes GA / PRA / PAA roles",
      "Every administrator is carved out of the control by design"
    )
  );
  if (gaExcluded.length) {
    push(
      findings,
      "High",
      "CA",
      `${gaExcluded.length} enforced All-users policy(ies) exclude Global / Privileged Role Administrator: ${gaExcluded.map((s) => s.name).join(" | ")} — administrators are outside MFA / block controls`
    );
  }
  ...
  // Session controls (D2) on population policies.
  const popPolicies = enforcedScopes.filter((s) => s.allUsers && (s.allApps || s.office365));
  const withSif = popPolicies.filter((s) => s.session.signInFrequency);
  const withPb = popPolicies.filter((s) => s.session.persistentBrowser === "never");
  const withTp = enforcedScopes.filter((s) => s.session.tokenProtection);
  const resilienceOff = enforcedScopes.filter((s) => s.session.disableResilienceDefaults);
  const sessionStatus =
    withSif.length && withPb.length
      ? CHECK.PASS
      : withSif.length || withPb.length || withTp.length
        ? CHECK.PARTIAL
        : CHECK.FAIL;
  checklist.push(
    checklistRow(
      "AP.CA.SessionControls",
      "Session controls: sign-in frequency + never-persistent browser on population policies",
      sessionStatus,
      "Medium",
      `signInFrequency: ${withSif.length ? withSif.map((s) => `${s.name}=${s.session.signInFrequency}`).join(", ") : "none"}; ` +
        `persistentBrowser=never: ${withPb.length ? namesOf(withPb) : "none"}; ` +
        `tokenProtection: ${withTp.length ? namesOf(withTp) : "none"}` +
        (resilienceOff.length ? `; resilience defaults disabled on ${namesOf(resilienceOff)}` : ""),
      "Stolen session cookies / refresh tokens stay valid for weeks"
    )
  );
  if (sessionStatus !== CHECK.PASS) {
    push(
      findings,
      "Medium",
      "CA",
      sessionStatus === CHECK.FAIL
        ? "No population CA policy sets sign-in frequency or a never-persistent browser session — stolen browser sessions stay valid until token expiry"
        : "Session controls are only partially applied (sign-in frequency / persistent browser / token protection) — 40_AttackPath_Checklist.csv"
    );
  }
  if (resilienceOff.length) {
    push(
      findings,
      "Info",
      "CA",
      `disableResilienceDefaults=true on ${namesOf(resilienceOff)} — an Entra outage will deny access instead of extending sessions`
    );
  }

  // OR grant MFA|compliant
  const orBypass = enforcedScopes.filter(
    (s) => s.grants.operator === "OR" && s.grants.controls.includes("mfa") && s.grants.requiresCompliantDevice
  );
  if (orBypass.length) {
    checklist.push(
      checklistRow(
    ...
    push(
      ...
      "High",
      "CA",
      `${orBypass.length} enforced CA use OR(mfa, compliant/domainJoined) — MFA bypasses device control: ${namesOf(orBypass)}`
    );
  } else {
    checklist.push(
      checklistRow(
  ...
  // Credentials on Microsoft first-party service principals (persistence backdoor).
  {
    const n = Number(summary.spCredentialsOnFirstParty);
    if (missing("servicePrincipalInventory") || !Number.isFinite(n)) {
      checklist.push(
        checklistRow(
          "AP.App.FirstPartyCreds",
          "No credentials on Microsoft first-party service principals",
          CHECK.NOT_EVALUATED,
          "High",
          "Not evaluated — service-principal inventory could not be collected",
          "Secret added to a first-party SP = silent standing access",
          { rationale: "servicePrincipals export failed" }
        )
      );
    } else {
      checklist.push(
        checklistRow(
          "AP.App.FirstPartyCreds",
          "No credentials on Microsoft first-party service principals",
          n ? CHECK.FAIL : CHECK.PASS,
          "High",
          n
            ? `${n} credential(s) on first-party SPs — 04_SPN_Credentials.csv`
            : `No tenant-added credentials on first-party SPs (${summary.servicePrincipalCount ?? "?"} SPs checked)`,
          "Secret added to a first-party SP = silent standing access"
        )
      );
    }
  ...
  // Delegated grants consented for all users with write / mailbox scopes.
  {
    const n = Number(summary.delegatedGrantsRisky);
    if (missing("delegatedGrants") || !Number.isFinite(n)) {
      checklist.push(
        checklistRow(
          "AP.App.DelegatedGrants",
          "No broad admin-consented delegated grants to third-party apps",
          CHECK.NOT_EVALUATED,
          "Medium",
          "Not evaluated — oauth2PermissionGrants could not be collected",
          "Every user's token can be exchanged for mailbox / files / directory write",
          { rationale: "oauth2PermissionGrants export failed" }
        )
      );
    } else {
      checklist.push(
        checklistRow(
          ...
          "Medium",
          n
            ? `${n} third-party app(s) with Critical/High delegated scopes for all users — 04_Delegated_Grants_AllPrincipals.csv`
            : `No risky AllPrincipals grants (${summary.delegatedGrantsAllPrincipals ?? 0} grants reviewed)`,
          "Consent-phishing style access without a new app"
        )
      );
    }
  }
