...
async function collectAttackPathChecks(graph, io, findings, summary, ctx = {}) {
  ...
  for (const gid of excludeGroupIds) {
    ...
    const usedBy = enforced
      .filter((p) => (usersCond(p).excludeGroups || []).includes(gid))
      .map((p) => p.displayName);
    groupMeta[gid] = {
  ...
  const partialEvidence = (v, fallback) =>
    v.status === "partial"
      ? `Only scoped / carved-out coverage: ${v.reasons.join("; ") || v.names.join(" | ")}` +
        (v.reportOnly.length && !v.partial.length
          ? ` (report-only: ${namesOf(v.reportOnly)})`
          : "")
      : fallback;
  ...
  const azureV = verdict(
    (s) => includesApp(s, APP.AZURE_MANAGEMENT) && s.grants.requiresMfa && targetsAdmins(s),
    { population: adminPop }
  );
  const portalsV = verdict(
    (s) => includesApp(s, APP.MICROSOFT_ADMIN_PORTALS) && s.grants.requiresMfa && targetsAdmins(s),
    { population: adminPop }
  );
  const azureCombined = {
    status:
  ...
  if (azureCombined.status === "none") {
    ...
    const portalsOnly = azureV.status === "none" && portalsV.status === "full";
    checklist.push(
      checklistRow(
        "AP.CA.AzureMgmt",
        "CA must require MFA for Azure management / admin portals",
        ...
        "High",
        portalsOnly
          ? `Admin portals only: ${portalsV.names.join(" | ")} — Azure Service Management API (ARM / CLI / PowerShell) not covered`
  ...
  const guestV = verdict(
    (s) => s.includesGuests && s.grants.requiresMfa && (s.allApps || s.office365),
  ...
  const adminPrV = verdict((s) => targetsGlobalAdmin(s) && s.grants.phishingResistant, {
    population: adminPrPop,
  });
  const adminMfaV = verdict((s) => targetsGlobalAdmin(s) && s.grants.requiresMfa, {
    population: adminPrPop,
  });
  coverageRow(
