...
const { decorateFindings, SCHEMA_VERSION } = require("./findings");

/**
 * userRegistrationDetails.methodsRegistered values that are phishing-resistant
 * on their own (Microsoft's "Phishing-resistant MFA" strength): passkeys /
 * FIDO2, Windows Hello for Business, platform SSO secure enclave keys and
 * certificate-based authentication. Hardware OATH tokens and Authenticator
 * passwordless phone sign-in are strong, but phishable — reported separately.
 */
const PHISHING_RESISTANT_RE =
  /^(passKeyDeviceBound.*|passKeyDeviceUnbound.*|fido2.*|windowsHelloForBusiness|macOsSecureEnclaveKey|x509Certificate.*|certificateBasedAuth.*)$/i;
const AUTHENTICATOR_PASSWORDLESS_RE = /^microsoftAuthenticatorPasswordless$/i;

function isPhishingResistantMethod(m) {
  return PHISHING_RESISTANT_RE.test(String(m || "").trim());
}
function isAuthenticatorPasswordless(m) {
  return AUTHENTICATOR_PASSWORDLESS_RE.test(String(m || "").trim());
}
...
async function runCollection(pool, outDir, opts = {}) {
  ...
  if (policies) {
    ...
    // Every policy that mentions the flow goes to the CSV; only *enforced*
    // ones count — a disabled or report-only policy is not a control.
    const deviceCodeAny = rows.filter((r) => r.AuthFlows);
    const deviceCode = deviceCodeAny.filter((r) => r.IsEnforced === "Yes");
    io.saveCsv("02_DeviceCode_CAPs.csv", deviceCodeAny);
    ...
    summary.caDeviceCodePoliciesAnyState = deviceCodeAny.length;
    ...
    // Device-code coverage itself is scored in attackpath (AP.CA.DeviceCode) on
    // the effective scope; only note here when a policy exists but is off.
    if (!deviceCode.length && deviceCodeAny.length) {
      findings.push({
        Severity: "Medium",
        Area: "CA",
        Detail: `${deviceCodeAny.length} device-code / auth-transfer policy(ies) exist but none is enforced (disabled or report-only): ${deviceCodeAny.map((r) => `${r.PolicyName} [${r.State}]`).join(" | ")}`,
      });
    }
  } else {
  ...
  if (capAnalyzerOffline && policies) {
    ...
    if (pack?.membershipPack) {
      ...
      const orphans = pack.membershipPack.orphans || [];
      summary.caOrphanUserReferences = orphans.length;
      if (orphans.length) {
        // Which policies still point at the deleted objects, and in which slot.
        const orphanSet = new Set(orphans);
        const refs = [];
        for (const p of policies) {
          const u = (p.conditions && p.conditions.users) || {};
          const inc = (u.includeUsers || []).filter((x) => orphanSet.has(x)).length;
          const exc = (u.excludeUsers || []).filter((x) => orphanSet.has(x)).length;
          if (inc || exc) {
            refs.push({
              PolicyName: p.displayName,
              State: p.state,
              OrphanIncludeUsers: inc,
              OrphanExcludeUsers: exc,
            });
          }
        }
        io.saveCsv("02_CA_Orphan_References.csv", refs);
        const excludeSlots = refs.reduce((n, r) => n + r.OrphanExcludeUsers, 0);
        findings.push({
          Severity: excludeSlots ? "Medium" : "Low",
          Area: "CA",
          Detail:
            `${orphans.length} user id(s) referenced by Conditional Access no longer exist in the directory (${refs.length} policy(ies): ${refs.slice(0, 4).map((r) => r.PolicyName).join(" | ")}${refs.length > 4 ? " …" : ""})` +
            (excludeSlots
              ? ` — ${excludeSlots} of them sit in exclude lists: a recreated account with the same id is impossible, but stale exclusions hide the policy's real scope and confuse reviews`
              : "") +
            " — clean the policy scopes — 02_CA_Orphan_References.csv",
        });
      }
    }
    if (pack?.signInPack) {
  ...
  // Users first: the registration report has no accountEnabled, and a few
  // hundred disabled leavers otherwise inflate every "without MFA" number.
  const usersSignIn = await soft(
    "usersSignInActivity",
    () =>
      graph.getAll(
        `${graph.GRAPH_BETA}/users?$select=id,displayName,userPrincipalName,accountEnabled,userType,createdDateTime,onPremisesSyncEnabled,signInActivity&$top=999`
      ),
    io
  );
  const userById = new Map();
  const userByUpn = new Map();
  if (Array.isArray(usersSignIn)) {
    for (const u of usersSignIn) {
      if (u.id) userById.set(u.id, u);
      if (u.userPrincipalName) userByUpn.set(String(u.userPrincipalName).toLowerCase(), u);
    }
  }
  const userFor = (reg) =>
    userById.get(reg.id) ||
    userByUpn.get(String(reg.userPrincipalName || "").toLowerCase()) ||
    null;
  ...
  if (regDetails) {
    ...
    const enabledKnown = userById.size > 0;
    const isEnabled = (reg) => {
      const u = userFor(reg);
      return u ? u.accountEnabled !== false : null; // null = unknown
    };

    const noMfaAll = regDetails.filter((u) => u.isMfaRegistered === false);
    const noMfa = noMfaAll.filter((u) => isEnabled(u) !== false);
    const noMfaDisabled = noMfaAll.filter((u) => isEnabled(u) === false);
    const withMfa = regDetails.filter((u) => u.isMfaRegistered === true);
    const phishingResistant = regDetails.filter((u) =>
      (u.methodsRegistered || []).some(isPhishingResistantMethod)
    );
    const authenticatorPasswordless = regDetails.filter(
      (u) =>
        (u.methodsRegistered || []).some(isAuthenticatorPasswordless) &&
        !(u.methodsRegistered || []).some(isPhishingResistantMethod)
    );
    ...
    const regRow = (u) => ({
      ...
      IsAdmin: u.isAdmin,
      AccountEnabled: isEnabled(u) === null ? "" : isEnabled(u),
      UserType: (userFor(u) && userFor(u).userType) || "",
      IsSsprRegistered: u.isSsprRegistered,
      MethodsRegistered: (u.methodsRegistered || []).join(", "),
      LastUpdated: u.lastUpdatedDateTime,
    });
    io.saveCsv("07_Users_Without_MFA.csv", noMfa.map(regRow));
    io.saveCsv("07_Users_Without_MFA_Disabled.csv", noMfaDisabled.map(regRow));
    io.saveCsv(
      "07_Users_PhishingResistant_or_Passkey.csv",
      phishingResistant.map((u) => ({
        UPN: u.userPrincipalName,
        DisplayName: u.userDisplayName,
        IsAdmin: u.isAdmin,
        AccountEnabled: isEnabled(u) === null ? "" : isEnabled(u),
        PhishingResistantMethods: (u.methodsRegistered || []).filter(isPhishingResistantMethod).join(", "),
        MethodsRegistered: (u.methodsRegistered || []).join(", "),
        IsMfaRegistered: u.isMfaRegistered,
      }))
    );
    io.saveCsv(
      "07_Users_AuthenticatorPasswordless.csv",
      authenticatorPasswordless.map((u) => ({
        UPN: u.userPrincipalName,
        DisplayName: u.userDisplayName,
        IsAdmin: u.isAdmin,
        MethodsRegistered: (u.methodsRegistered || []).join(", "),
    ...
    summary.usersWithoutMfaDisabled = noMfaDisabled.length;
    summary.usersWithoutMfaEnabledKnown = enabledKnown;
    summary.usersWithMfa = withMfa.length;
    summary.usersPhishingResistant = phishingResistant.length;
    summary.usersAuthenticatorPasswordless = authenticatorPasswordless.length;
    // Kept for older readers; now strictly phishing-resistant.
    summary.usersWithPasskeyOrPhishingResistant = phishingResistant.length;
    summary.adminsWithoutMfa = noMfa.filter((u) => u.isAdmin).length;
    summary.adminsPhishingResistant = phishingResistant.filter((u) => u.isAdmin).length;
    ...
    if (noMfa.length) {
      ...
      findings.push({
        ...
        Area: "MFA",
        Detail:
          `${noMfa.length} ${enabledKnown ? "enabled " : ""}users without MFA registered (${adminRows.length} admins` +
          (humanAdmins.length !== adminRows.length
            ? `, ${humanAdmins.length} human`
            : "") +
          `)` +
          (noMfaDisabled.length
            ? `; ${noMfaDisabled.length} further disabled accounts excluded (07_Users_Without_MFA_Disabled.csv)`
            : enabledKnown
              ? ""
              : "; account state unknown (users export failed)") +
    ...
    findings.push({
      ...
      Area: "Passkey",
      Detail:
        `${phishingResistant.length} users with phishing-resistant methods (passkey / FIDO2 / WHfB / CBA), ${summary.adminsPhishingResistant} of them admins` +
        (authenticatorPasswordless.length
          ? `; ${authenticatorPasswordless.length} rely on Authenticator passwordless only (strong, not phishing-resistant)`
          : "") +
        " — 07_Users_PhishingResistant_or_Passkey.csv",
    });
  }

  // Inactive users via beta signInActivity. "Last seen" is the newest of the
  // interactive, non-interactive and last-successful timestamps — a mailbox
  // that refreshes tokens daily is not dormant just because nobody typed a
  // password this quarter.
  if (usersSignIn) {
    const cutoff = daysAgo(inactiveDays);
    const graceCutoff = daysAgo(Math.min(inactiveDays, 14));
    const inactive = [];
    for (const u of usersSignIn) {
      if (u.accountEnabled === false) continue;
      const sa = u.signInActivity || {};
      const stamps = [
        parseDate(sa.lastSignInDateTime),
        parseDate(sa.lastNonInteractiveSignInDateTime),
        parseDate(sa.lastSuccessfulSignInDateTime),
      ].filter((t) => t !== null);
      const last = stamps.length ? Math.max(...stamps) : null;
      const created = parseDate(u.createdDateTime);
      // Freshly created accounts have not had time to sign in.
      if (last === null && created !== null && created > graceCutoff) continue;
      if (last === null || last < cutoff) {
        const interactive = parseDate(sa.lastSignInDateTime);
        inactive.push({
          DisplayName: u.displayName,
          UPN: u.userPrincipalName,
          UserType: u.userType,
          AccountEnabled: u.accountEnabled,
          OnPremSynced: u.onPremisesSyncEnabled === true,
          LastInteractive: sa.lastSignInDateTime || "Never",
          LastNonInteractive: sa.lastNonInteractiveSignInDateTime || "Never",
          LastSuccessful: sa.lastSuccessfulSignInDateTime || "",
          LastAny: last ? new Date(last).toISOString() : "Never",
          Created: u.createdDateTime,
          DaysSinceInteractive: interactive
            ? Math.floor((Date.now() - interactive) / 86400000)
            : "Never",
          DaysSinceAny: last ? Math.floor((Date.now() - last) / 86400000) : "Never",
        });
      }
    }
    ...
    findings.push({
      ...
      Detail: `${inactive.length} enabled accounts with no sign-in of any kind since ${inactiveDays} days — 08_Accounts_Inactive_${inactiveDays}d.csv`,
    });
  }
  ...
  if (deviceReg) {
    ...
    // Who becomes local administrator on every Entra-joined device (D7).
    const localAdmins = (deviceReg.azureADJoin && deviceReg.azureADJoin.localAdmins) || {};
    const gaLocalAdmin = localAdmins.enableGlobalAdmins !== false; // default true
    const registeringLocalAdmin =
      (localAdmins.registeringUsers && localAdmins.registeringUsers["@odata.type"]) || "";
    const registeringUsersLocalAdmin = /allDeviceRegistrationMembership/i.test(registeringLocalAdmin)
      ? "All registering users"
      : /noDeviceRegistrationMembership/i.test(registeringLocalAdmin)
        ? "None"
        : registeringLocalAdmin
          ? "Selected"
          : "unknown";
    const laps = deviceReg.localAdminPassword && deviceReg.localAdminPassword.isEnabled;
    summary.deviceJoinGlobalAdminsLocalAdmin = gaLocalAdmin;
    summary.deviceJoinRegisteringUsersLocalAdmin = registeringUsersLocalAdmin;
    summary.deviceJoinAdminConfigurable = deviceReg.azureADJoin
      ? deviceReg.azureADJoin.isAdminConfigurable
      : null;
    summary.deviceLapsEnabled = laps == null ? null : laps === true;
    if (gaLocalAdmin) {
      findings.push({
        Severity: "Medium",
        Area: "DeviceJoin",
        Detail:
          "Global Administrators are local administrators on every Entra-joined device (azureADJoin.localAdmins.enableGlobalAdmins=true) — a GA token or PRT theft on any workstation, and any GA phish, becomes endpoint admin. Disable and use a dedicated Cloud Device Administrator / LAPS flow.",
      });
    }
    if (registeringUsersLocalAdmin === "All registering users") {
      findings.push({
        Severity: "Medium",
        Area: "DeviceJoin",
        Detail:
          "Every user who joins a device becomes its local administrator (localAdmins.registeringUsers=all) — standard users run as admin on their own PC; scope to a group or none.",
      });
    }
    if (laps === false) {
      findings.push({
        ...
        Area: "DeviceJoin",
        Detail:
          "Windows LAPS is disabled at the device registration policy (localAdminPassword.isEnabled=false) — local admin passwords cannot be rotated / escrowed for Entra-joined devices.",
      });
    }
...
`.trim(),
  ...
  if (domains) {
    ...
    // Federated domains (D4): the external IdP owns authentication for these
    // users. Its signing certificate and MFA-trust setting decide whether a
    // forged SAML token (Golden SAML) or a phished federated MFA is accepted.
    const federated = domains.filter((d) => /federated/i.test(String(d.authenticationType || "")));
    const unverified = domains.filter((d) => d.isVerified === false);
    summary.domainsTotal = domains.length;
    summary.domainsFederated = federated.length;
    summary.domainsUnverified = unverified.length;
    const fedRows = [];
    for (const d of federated.slice(0, 25)) {
      const cfg = await soft(
        `federation_${String(d.id).replace(/[^a-z0-9]/gi, "_")}`,
        () => graph.getAll(`${graph.GRAPH}/domains/${encodeURIComponent(d.id)}/federationConfiguration`),
        io
      );
      for (const f of Array.isArray(cfg) ? cfg : []) {
        const certExpiry = f.signingCertificateUpdateStatus && f.signingCertificateUpdateStatus.certificateUpdateResult;
        fedRows.push({
          Domain: d.id,
          IssuerUri: f.issuerUri || "",
          PassiveSignInUri: f.passiveSignInUri || "",
          FederatedIdpMfaBehavior: f.federatedIdpMfaBehavior || "",
          IsSignedAuthenticationRequestRequired: f.isSignedAuthenticationRequestRequired,
          PreferredAuthenticationProtocol: f.preferredAuthenticationProtocol || "",
          SigningCertificate: f.signingCertificate ? "present" : "",
          NextSigningCertificate: f.nextSigningCertificate ? "present" : "",
          CertUpdateResult: certExpiry || "",
          PromptLoginBehavior: f.promptLoginBehavior || "",
        });
      }
    }
    io.saveCsv("12_Federation.csv", fedRows);
    if (federated.length) {
      const acceptsIdpMfa = fedRows.filter((r) =>
        /acceptIfMfaDoneByFederatedIdp|enforceMfaByFederatedIdp/i.test(r.FederatedIdpMfaBehavior)
      );
      const unsignedReq = fedRows.filter((r) => r.IsSignedAuthenticationRequestRequired === false);
      findings.push({
        Severity: acceptsIdpMfa.length ? "High" : "Medium",
        Area: "Federation",
        Detail:
          `${federated.length} federated domain(s) (${federated.map((d) => d.id).slice(0, 4).join(", ")}) — authentication is delegated to an external IdP; a compromised IdP signing key mints tokens for any user (Golden SAML)` +
          (acceptsIdpMfa.length
            ? `; ${acceptsIdpMfa.length} accept the IdP's MFA claim (${acceptsIdpMfa.map((r) => `${r.Domain}=${r.FederatedIdpMfaBehavior}`).join(", ")}) so Entra CA MFA is satisfied by whatever the IdP asserts`
            : "; federatedIdpMfaBehavior does not accept the IdP MFA claim (Entra re-challenges) or was not readable") +
          (unsignedReq.length ? `; ${unsignedReq.length} do not require signed authentication requests` : "") +
          " — 12_Federation.csv",
      });
    }
    if (unverified.length) {
      findings.push({
        Severity: "Low",
        Area: "Federation",
        Detail: `${unverified.length} unverified domain(s) left in the tenant (${unverified.map((d) => d.id).slice(0, 5).join(", ")}) — remove stale domains; a verified takeover of a lapsed DNS name would make them usable`,
      });
    }
  }
  ...
  // ── Write summary ─────────────────────────────────────────────────────
  summary.findingsSchemaVersion = SCHEMA_VERSION;
  io.saveJson("00_SUMMARY.json", summary);
  io.saveCsv("00_Findings.csv", decorateFindings(findings));
