...
const { aggregateFailuresByIp, isCopilotOrAgentUpn } = require("./posture");
...
async function collectSignInAndAuditInsights(graph, io, findings, summary, opts = {}) {
  ...
  if (huntNoMfa && huntNoMfa.results) {
    ...
    const humanish = huntNoMfa.results.filter((r) => {
      ...
      if (isCopilotOrAgentUpn(upn)) return false;
      if (/\b(salle|salon|room|meeting)\b/i.test(upn)) return false;
      if (/^(svc[-._]|service[-._]|spn[-._])/i.test(String(upn).split("@")[0] || "")) {
        return false;
      }
      return true;
    });
