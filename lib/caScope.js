...
function effectiveScope(p, { roleDefMap = {}, authStrengths = [], groupMeta = {} } = {}) {
  ...
  // Complementary groups (the include target of a sibling policy with the same
  // grant) are scoping, not carve-outs — leave them out of the "broad" test.
  const carvedGroups = excludedGroupMeta.filter((m) => !m.complementary);
  const excludedGroupMembers = carvedGroups.reduce(
    (n, m) => n + (Number.isFinite(Number(m.memberCount)) ? Number(m.memberCount) : 0),
    0
  );
  const excludedMembersKnown = carvedGroups.some((m) => Number.isFinite(Number(m.memberCount)));
