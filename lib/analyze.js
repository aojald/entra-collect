...
const { detectMfaCoverageFromPolicies } = require("./caScope");
const { SCHEMA_VERSION } = require("./findings");
const { runExtraRules, buildAttackChain } = require("./narrativesExtra");
...
/**
 * Coverage verdict for one CA control: true / false / null (unknown or n/a).
 * A run where the CA export failed cannot yield `false` — that would turn an
 * outage into "no policy".
 */
function caCovered(caCoverage, controlRe, summary = null) {
  if (summary && summary.caPoliciesCollected === false) return null;
  ...
  // "partial" = a policy exists but is scoped / carved out: the gap is real.
  if (v === "false" || v === "no" || v === "0" || v === "partial") return false;
  return null;
}

/**
 * True when the collector actually measured a sign-in count for the given
 * summary key family (e.g. "deviceCodeSignIns" → 90d / 30d). `null` means the
 * source was unavailable, and an absent key means the step never ran; neither
 * may be read as zero.
 */
function signInCountKnown(summary, prefix) {
  for (const k of Object.keys(summary || {})) {
    if (!k.startsWith(prefix) || !/\d+d$/.test(k)) continue;
    const v = summary[k];
...
function detectLicenseGatedAccess(caAudit) {
  ...
  const hasBlockExcludingLicenses = enforced.some((r) => {
    ...
    return (
      ...
      (/licen/i.test(excl) || looksLikeUnlicensedPolicyText(name))
    );
  });
...
/** Guest vs member from Graph userType, with #EXT# as fallback only. */
function isGuestAccount(r) {
  const t = String(r?.UserType || r?.userType || "").toLowerCase();
  if (t === "guest") return true;
  if (t === "member") return false;
  return /#ext#/i.test(
    String(r?.UPN || r?.UserPrincipalName || r?.userPrincipalName || "")
  );
}

function summarizeNoMfaBreakdown(noMfaRows) {
  const buckets = { human: 0, room: 0, service: 0, shared: 0, guest: 0 };
...
function pushNarrative(out, n) {
  ...
  out.push({
    ...
    Status: n.status || (/^info$/i.test(severity) ? "Info" : "Fail"),
    Confidence: n.confidence || "High",
    LicenceRequired: n.licence || "",
...
function loadContext(outDir) {
  ...
  // Raw policies let the rules reason on effective scope instead of labels.
  const caPoliciesRaw = readJson(path.join(outDir, "02_ca_policies_raw.json"));
  const authStrengthsRaw = readJson(path.join(outDir, "01_authentication_strengths.json"));
  const roleDefsRaw = readJson(path.join(outDir, "03_role_definitions.json"));
  ...
  const spCredentials = readCsv(path.join(outDir, "04_SPN_Credentials.csv"));
  const delegatedGrants = readCsv(
    path.join(outDir, "04_Delegated_Grants_AllPrincipals.csv")
  );
  const appLongLived = readCsv(path.join(outDir, "04_App_LongLived_Secrets.csv"));
  const authMethodsCsv = readCsv(path.join(outDir, "01_auth_methods.csv"));
  const regDetailsRaw = readJson(path.join(outDir, "07_user_registration_details.json"));
  const crossTenantTrust = readCsv(path.join(outDir, "15_CrossTenant_Trust.csv"));
  const caOrphanRefs = readCsv(path.join(outDir, "02_CA_Orphan_References.csv"));
  const adminTooling = pickNonEmpty(outDir, [
    "26_AdminTooling_SignIns_90d.csv",
    "26_AdminTooling_SignIns_30d.csv",
  ]);
  ...
  return {
    ...
    riskDetections,
    caAudit,
    caPoliciesRaw: Array.isArray(caPoliciesRaw) ? caPoliciesRaw : null,
    authStrengthsRaw: Array.isArray(authStrengthsRaw) ? authStrengthsRaw : [],
    roleDefMap: Array.isArray(roleDefsRaw)
      ? Object.fromEntries(roleDefsRaw.map((d) => [d.id, d.displayName]))
      : {},
    exclGroups,
    rmmFamily,
    rmmAssets,
    ...
    appLongLived,
    authMethodsCsv,
    regDetailsRaw: Array.isArray(regDetailsRaw) ? regDetailsRaw : [],
    crossTenantTrust,
    caOrphanRefs,
    adminTooling,
    aiAgents,
...
function buildPrivIndex(priv, privHygiene) {
  ...
  // Standing = permanent, tenant-wide. PIM-eligible, activated-at-collection
  // and AU/app-scoped rows are recorded as roles but do not make the account
  // a "GA" for the live-path narratives.
  const standing = (r) =>
    /^permanent/i.test(String(r.AssignmentType || "Permanent")) &&
    (!r.Scope || /^tenant$/i.test(String(r.Scope)));
  for (const r of priv || []) {
    const upn = r.UPNOrAppId || r.UPN || "";
    if (!/@/.test(upn)) continue;
    if (/^group$/i.test(r.PrincipalType || "")) continue;
    const e = ensure(upn);
    if (!e) continue;
    const role = r.RoleName || "";
    if (role) e.roles.add(role);
    if (/^Global Administrator$/i.test(role) && standing(r)) e.isGa = true;
  }
  ...
  for (const r of privHygiene || []) {
    ...
    // Newer hygiene rows carry StandingRoles / IsGlobalAdmin (standing only);
    // older folders only have Roles, where GA may include eligible.
    const hasStandingCol = r.StandingRoles !== undefined;
    if (truthy(r.IsGlobalAdmin)) e.isGa = true;
    else if (!hasStandingCol && roles.some((x) => /^Global Administrator$/i.test(x))) {
      e.isGa = true;
    }
...
function runRules(ctx) {
  ...
  const cov = (re) => caCovered(ctx.caCoverage, re, ctx.summary);
  const deviceCodeBlocked = cov(/device.?code|auth.?transfer/i);
  const legacyBlocked = cov(/legacy/i);
  const userRiskCa = cov(/user.?risk|high.?risk.?user|password change for high risk/i);
  const guestMfaCa = cov(/guest/i);
  const secInfoCa = cov(/security.?info|register/i);
  ...
  // ── NARR.DeviceCode.Observed — High if usage + no CA (and not already covered only by priv) ─
  {
    ...
    if (users.length && deviceCodeBlocked === false) {
      ...
      // Distinguish "we looked and saw none" from "we could not look".
      const usageKnown = signInCountKnown(ctx.summary, "deviceCodeSignIns");
      const partial = /partial/i.test(
        String(ctx.summary.deviceCodeSource90d || ctx.summary.deviceCodeSource30d || "")
      );
      pushNarrative(narratives, {
        id: "NARR.DeviceCode.NoCA",
        severity: "High",
        title: usageKnown
          ? "No CA blocking device-code (no usage observed in window)"
          : "No CA blocking device-code (usage unknown — sign-in logs not readable)",
        narrative:
          (usageKnown
            ? `No device-code sign-ins were found in the ${partial ? "collected part of the " : ""}window, but Conditional Access still does not block device code / auth transfer — the gap remains exploitable.`
            : "Device-code usage could not be measured (sign-in logs were not readable and no identity hunting table was available), and Conditional Access does not block device code / auth transfer — treat the flow as open until a log source confirms otherwise.") +
          (partial ? " Sign-in coverage was partial (page budget reached)." : ""),
        evidence:
          "Covered=false on device-code / auth-transfer control" +
          (usageKnown ? "" : "; deviceCodeSignIns90d=unknown"),
        remediation:
          "Enforce CA block on deviceCodeFlow + authenticationTransfer before the flow is abused." +
          (usageKnown ? "" : " Grant AuditLog.Read.All (or a hunting table) to measure current usage first."),
        relatedFiles: ["40_CA_AttackPath_Coverage.csv", "20_DeviceCode_SignIns_90d.csv"],
        relatedChecks: ["AP.CA device code coverage"],
        confidence: usageKnown && !partial ? "High" : "Medium",
      });
    }
  }
  ...
  {
    // Standing = permanent assignment (not PIM-eligible, not a PIM activation
    // that happened to be live during the run). Group shells are skipped: the
    // collector already expanded their members into their own rows.
    const gaRows = (ctx.priv || []).filter(
      (r) =>
        /^Global Administrator$/i.test(r.RoleName || "") &&
        !/^group$/i.test(r.PrincipalType || "")
    );
    const isStanding = (r) => /^permanent/i.test(String(r.AssignmentType || "Permanent"));
    const isActivated = (r) => /activated|pim — active/i.test(String(r.AssignmentType || ""));
    ...
    if (permanent >= 5 || live >= 5) {
      ...
      evidence.push(
        `Total permanent GA assignments: ${permanent || all.length} — ${enabledUsers.length} enabled user(s), ${disabledUsers.length} disabled user(s), ${apps.length} service principal(s); PIM eligible rows: ${ctx.summary.pimEligibleCount ?? "n/a"}` +
          (activated.size ? `; ${activated.size} PIM activation(s) live at collection time (not counted)` : "")
      );
      for (const g of enabledUsers.slice(0, 15)) {
        const who =
          g.label && g.label.toLowerCase() !== g.id ? `${g.label} <${g.id}>` : g.id;
        evidence.push(
          `enabled user — ${who}${g.synced ? " (hybrid-synced)" : ""}${g.viaGroup ? ` (via group ${g.viaGroup})` : ""} [${g.assignment}]`
        );
      }
    ...
    if (exposed.length >= 10 && noMfaSet.size) {
      ...
      const unlicensedBlock = enforcedCa.filter(
        (r) =>
          ...
          looksLikeUnlicensedPolicyText(r.PolicyName)
      );
  ...
  // ── NARR.Legacy.Observed+NoCA ─
  {
    ...
    if (legacyFail && legacyRows.length) {
      ...
      const usageKnown = signInCountKnown(ctx.summary, "legacyAuth");
      const partial = /partial/i.test(String(ctx.summary.legacyAuthSource90d || ""));
      const legacySt = checklistStatus(ctx.checklist, "AP.CA.LegacyBlock");
      const legacyRow = (ctx.checklist || []).find(
        (r) => String(r.CheckId || "").toUpperCase() === "AP.CA.LEGACYBLOCK"
      );
      const ev = String(legacyRow?.Evidence || "");
      const reportOnly =
        /partial/i.test(legacySt) || /report-only|report.?only|not enforced/i.test(ev);
      const reportOnlyNames = (ev.match(
        /(?:report-only[^.]*?:\s*)([^|]+(?:\s*\|\s*[^|]+)*)/i
      ) || [])[1];
      pushNarrative(narratives, {
        id: "NARR.Legacy.NoCA",
        severity: "High",
        title: reportOnly
          ? "Legacy-auth block exists only in report-only — it stops nothing"
          : "Legacy authentication not blocked by Conditional Access",
        impact:
          "Password spray / basic auth (SMTP, ActiveSync, IMAP) never prompts MFA. A leaked password is a session.",
        narrative: reportOnly
          ? `A legacy-auth block is already written but left in report-only${reportOnlyNames ? ` (${reportOnlyNames.trim()})` : ""}. Report-only policies do not deny a single request — the spray path is fully open. Turning the existing policy On is the fix, not designing another one.` +
            (usageKnown
              ? ` No successful legacy-auth sign-in was seen in the ${partial ? "collected part of the " : ""}window, which only means nobody used the path recently — not that it is closed.`
              : " Legacy-auth usage could not be measured, so you cannot see who would break if you flip it on.")
          : usageKnown
            ? `No successful legacy-auth sign-ins were found in the ${partial ? "collected part of the " : ""}window, but CA still does not block legacy clients — the spray path remains open.`
            : "Legacy-auth usage could not be measured (sign-in logs not readable, no identity hunting table), and CA does not block legacy clients — the spray path is open and unobserved.",
        evidence:
          `AP.CA.LegacyBlock=${legacySt || "Fail"}` +
          (ev ? `; ${ev}` : "") +
          (usageKnown ? "" : "; legacyAuthEvents90d=unknown"),
        remediation: reportOnly
          ? "1) Identify any remaining IMAP/SMTP/ActiveSync clients (printers, scanners, legacy mail relays). 2) Move those to modern auth or a connector. 3) Switch the existing report-only legacy-auth policy to On for all users — do not write a second policy. 4) Disable Authenticated SMTP where unused."
          : "1) Enforce a CA policy blocking Exchange ActiveSync + Other clients / legacy auth for all users. 2) Disable Authenticated SMTP where unused." +
            (usageKnown ? "" : " 3) Grant AuditLog.Read.All first if you need to see who still uses basic auth."),
        relatedFiles: ["40_AttackPath_Checklist.csv", "21_LegacyAuth_Success_90d.csv", "02_CA_Audit.csv"],
        relatedChecks: ["AP.CA.LegacyBlock"],
        confidence: usageKnown && !partial ? "High" : "Medium",
      });
    }
  ...
  // ── NARR.CA.ExclGroups — only groups that are a real bypass ─
  {
    const exclRows = ctx.exclGroups || [];
    const complementary = exclRows.filter(
      (r) =>
        truthy(r.Complementary) || /^complementary/i.test(String(r.Risk || ""))
    );
    const weak = exclRows.filter((r) => {
      if (truthy(r.Complementary) || /^complementary/i.test(String(r.Risk || ""))) {
        return false;
      }
      return (
    ...
    const weakCount = weak.length || (complementary.length ? 0 : inventoryExcl.length);
    if ((exclFail && weakCount >= 1) || weakCount >= 2) {
      const names = weak
        .slice(0, 8)
        .map((r) => {
          const n = r.DisplayName || r.displayName || "";
          const members = r.MemberCount != null ? ` (${r.MemberCount} members)` : "";
          return n ? n + members : "";
        })
        .filter(Boolean);
      const dyn = weak.filter((r) => truthy(r.Dynamic)).length;
      const travel = weak.filter((r) =>
        /travel|viaje|voyage|temporary|temp|exception/i.test(r.DisplayName || "")
      );
      pushNarrative(narratives, {
        id: "NARR.CA.ExclGroups",
        severity: weak.some((r) => Number(r.MemberCount) >= 50) || dyn ? "High" : "Medium",
        priority: weakCount >= 5 || dyn ? "Now" : "Next",
        title: `${weakCount} CA exclusion group(s) anyone can join — that is a standing MFA bypass`,
        impact:
          "A User Administrator or group owner adds themselves (or a planted account) to an exclusion group and signs in without the MFA / device / country policy.",
        narrative:
          `${weakCount} Conditional Access exclusion group(s) are not locked down (not role-assignable, or dynamic). ` +
          "Anyone who can change membership carves themselves out of the policy — this is not a compliance miss, it is the bypass an attacker uses after the first privileged foothold. " +
          (dyn
            ? `${dyn} of them ${dyn === 1 ? "is" : "are"} dynamic: an attribute change (title, department, display name) adds the account without a human approval. `
            : "") +
          (travel.length
            ? `${travel.length} look${travel.length === 1 ? "s" : ""} like travel / temporary exceptions (${travel.map((r) => r.DisplayName).join(", ")}) — those should be time-boxed PIM-style, not standing groups. `
            : "") +
          (complementary.length
            ? `${complementary.length} other group(s) appear in exclude lists but are the include target of a complementary policy (${complementary
                .map((r) => r.DisplayName)
                .slice(0, 3)
                .join(", ")}) — members are covered there, not bypassing. They are not in this count.`
            : ""),
        evidence:
          (names.length
            ? names.join(" | ") +
            ...
            : `${weakCount} groups`) +
          (complementary.length
            ? ` | complementary=${complementary.map((r) => r.DisplayName).join(", ")}`
            : "") +
          " — 40_CA_Exclusion_Groups.csv",
        remediation:
          "1) Convert every real exclusion group to role-assignable (RMAU) and remove standing owners. 2) Prefer named break-glass accounts over groups. 3) Replace dynamic exclusion with a static, reviewed list. 4) Time-box travel exceptions (access package / PIM, not a group that lives forever). 5) Alert on membership changes on these groups.",
        relatedFiles: [
          "40_CA_Exclusion_Groups.csv",
          "40_AttackPath_Checklist.csv",
        ],
  ...
  // ── NARR.MFA.MassGap — members vs guests; guests MFA at home ─
  {
    ...
    const guestRows = noMfaRows.filter(isGuestAccount);
    const memberRows = noMfaRows.filter((r) => !isGuestAccount(r));
    const guestCount = guestRows.length;
    const memberCount = memberRows.length || Math.max(0, n - guestCount);
    ...
    const inbound = (ctx.crossTenantTrust || []).find((r) =>
      /^default$/i.test(r.Scope || "")
    );
    const inboundMfaTrusted = inbound && truthy(inbound.TrustMfa);
    const guestPathOpen =
      guestCount >= 20 && inboundMfaTrusted && guestMfaCa === false;
    // Name-based licence gating is recorded as context only; it no longer
    // changes the verdict.
    const gate = detectLicenseGatedAccess(ctx.caAudit);
    const mfaCa = ctx.caPoliciesRaw
      ? detectMfaCoverageFromPolicies(ctx.caPoliciesRaw, {
          roleDefMap: ctx.roleDefMap,
          authStrengths: ctx.authStrengthsRaw,
        })
      : detectEnforcedMfaCoverage(ctx.caAudit);
    // Entra behaviour: an account with *no* registered method that hits an MFA
    // grant is sent to interrupt-mode registration and can enrol whatever the
    // attacker holds. All-users MFA therefore only compensates when Register
    // security info is itself protected and the policy is not carved out.
    const secInfoProtected = secInfoCa === true;
    const compensated = mfaCa.covered && secInfoProtected && !mfaCa.broadExclusions;
    const whyNotCompensated = !mfaCa.covered
      ? mfaCa.partial
        ? `MFA CA is scoped or carved out (${(mfaCa.reasons || []).slice(0, 2).join("; ") || mfaCa.evidence.slice(0, 2).join(", ")})`
        : "no enforced All-users / All-apps MFA policy"
      : !secInfoProtected
        ? "Register security info is not protected by CA, so a password-only account can enrol the attacker's method on first MFA prompt"
        : "population MFA policy has broad exclusions";
    const guestNote =
      guestCount > 0
        ? ` ${guestCount} guest account(s) also have no method registered *in this tenant* — that is expected (they MFA at home)` +
          (inboundMfaTrusted
            ? ", but inbound MFA trust is on, so whatever their home tenant (or a throw-away tenant) calls MFA satisfies your grant. See NARR.External.InboundTrust."
            : guestMfaCa === false
              ? ", and there is no guest MFA CA here — guests who can be phished at home walk in."
              : ".")
        : "";
    if (
      memberCount >= 20 ||
      ...
      if (humanAdminCount >= 1) {
        // Human admins without a registered method are Critical regardless of
        // CA: the first MFA prompt they (or an attacker) meet is a registration.
        severity = "Critical";
        priority = "Now";
        narrative =
          `${memberCount} member account(s) have no MFA method, including ${humanAdminCount} human admin(s). ` +
          (compensated
            ? `All-users MFA CA is enforced (${mfaCa.allUsersEvidence.slice(0, 2).join(", ")}) and security-info registration is protected, but an administrator with no method is still one guessed password away from an attacker registering their own Authenticator — register phishing-resistant methods for these accounts first.`
            : `Without population MFA that also protects registration (${whyNotCompensated}), these accounts are password-only spray targets and the first MFA prompt becomes the attacker's enrolment.`) +
          guestNote;
      } else if (compensated) {
        severity = memberCount >= 200 ? "Medium" : "Low";
        priority = "Later";
        narrative =
          `${memberCount} member account(s) have no MFA method registered, but this is a registration-inventory number rather than an open password-only path. ` +
          `Enforced All-users MFA is present (${mfaCa.allUsersEvidence.slice(0, 3).join(", ")}) and Register security info is protected, so a password alone cannot complete a sign-in or enrol a new method from an untrusted context. ` +
          "Residual exposure: MFA exclusion group members, legacy/basic auth if not blocked, and any user-action policy exemptions. " +
          "Do not run a campaign against the raw 07_Users_Without_MFA count." +
          guestNote;
        ...
        severity = memberCount >= 200 ? "High" : "Medium";
        priority = "Next";
        narrative =
          `${memberCount} member account(s) have no MFA method and the tenant does not compensate: ${whyNotCompensated}. ` +
          "Those are the password-spray / credential-stuffing targets — register a method or block sign-in." +
          guestNote;
        if (mfaCa.scoped) {
          narrative +=
            ` Group-scoped MFA exists (${mfaCa.scopedEvidence.slice(0, 2).join(", ")}) — confirm membership covers interactive members; this is not the same as All-users MFA.`;
        }
      } else if (guestPathOpen && memberCount < 20) {
        severity = "Medium";
        priority = "Next";
        narrative =
          `${guestCount} guest account(s) have no method here and inbound MFA trust is on with no guest MFA CA — a guest from a weak or attacker-controlled home tenant walks in on their home factor.` +
          (memberCount
            ? ` ${memberCount} member account(s) are also unregistered (${whyNotCompensated}).`
            : "");
      } else {
        severity = "Medium";
        priority = "Next";
        narrative =
          `${memberCount} member account(s) have no MFA method and the tenant does not fully compensate (${whyNotCompensated}).` +
          guestNote;
      }
      const title = humanAdminCount
        ? `${humanAdminCount} admin(s) and ${memberCount} members have no MFA method`
        : compensated
          ? `${memberCount} members have no MFA method (CA already requires it)`
          : guestPathOpen && memberCount < 20
            ? `${guestCount} guests have no MFA here — inbound trust accepts their home factor`
            : `${memberCount} member account(s) have no MFA method`;
      pushNarrative(narratives, {
        id: "NARR.MFA.MassGap",
        severity,
        priority,
        title,
        impact: humanAdminCount
          ? "A guessed admin password becomes the attacker's Authenticator on first prompt."
          : compensated
            ? "Not a password-only path today — leftover unregistered members and guests, not an open door."
            : guestPathOpen && memberCount < 20
              ? "A guest from a weak or attacker-created home tenant satisfies your MFA grant."
              : "Password spray against unregistered members completes without a second factor, then enrols the attacker's method.",
        narrative,
        evidence:
          `usersWithoutMfa=${n}; membersWithoutMfa=${memberCount}; guestsWithoutMfa=${guestCount}; guestHeavy=${guestCount > memberCount}; inboundMfaTrusted=${inboundMfaTrusted ? "true" : "false"}` +
          `; adminsWithoutMfa=${adminsReported}; humanAdminsWithoutMfa=${humanAdminCount}; serviceAdminsWithoutMfa=${serviceAdmins.length}` +
          ...
          (mfaCa.covered
            ...
            : mfaCa.partial
              ? `; allUsersMfaCA=partial (${(mfaCa.reasons || mfaCa.evidence).join(" | ")})`
              : "; allUsersMfaCA=false") +
          (mfaCa.scoped
            ? `; groupScopedMfaCA=true (${mfaCa.scopedEvidence.join(" | ")})`
            : "; groupScopedMfaCA=false") +
          `; secInfoRegistrationProtected=${secInfoCa === true ? "true" : secInfoCa === false ? "false" : "unknown"}` +
          `; compensated=${compensated}` +
          " — 07_Users_Without_MFA.csv",
        remediation: compensated
          ? "1) Do not run a blanket MFA campaign on the raw count. 2) Audit MFA / travel exclusion group membership (make those groups role-assignable). 3) Confirm legacy auth is blocked. 4) Register methods only for remaining interactive members; block sign-in on rooms/shared/service. 5) If inbound MFA trust is on, turn it off at the default and keep it only for named partners."
          : "1) Enforce All-users MFA on all cloud apps. 2) Add a Register-security-info policy that requires a trusted location, a compliant device, or a helpdesk TAP. 3) Register phishing-resistant methods for human admins first, then remaining interactive members. 4) Guests: turn off default inbound MFA trust (or add a guest MFA policy here) — do not enrol them in your tenant. 5) Block sign-in on shared/service objects rather than 'MFA' them.",
        relatedFiles: [
          "07_Users_Without_MFA.csv",
          "02_CA_Audit.csv",
          "40_CA_Exclusion_Groups.csv",
          "15_CrossTenant_Trust.csv",
        ],
        relatedChecks: ["AP.CA.SecInfoReg", "AP.CA.GuestMfa"],
        confidence: compensated || guestCount > memberCount ? "Medium" : "High",
        rationale:
          guestCount > memberCount
            ? "Raw count is guest-heavy; guests MFA at home — members drive the risk"
            : compensated
              ? "CA already requires MFA; leftover rows are inventory"
              : "Unregistered members plus unprotected registration",
      });
    }
  }
  ...
  // ── NARR.App.FirstPartyCredential — secret/cert added to a Microsoft first-party SP ─
  //
  // First-party applications authenticate with Microsoft's own keys; the
  // tenant never needs to add a credential to their service principal. When
  // one is there, someone put it there — and it grants whatever that app has
  // been consented, without an app registration to audit.
  {
    const rows = (ctx.spCredentials || []).filter((r) => truthy(r.MicrosoftOwned));
    if (rows.length) {
      const bySp = new Map();
      for (const r of rows) {
        const k = r.SPNDisplayName || r.ServicePrincipalId;
        if (!bySp.has(k)) bySp.set(k, []);
        bySp.get(k).push(r);
      }
      const active = rows.filter((r) => !truthy(r.Expired));
      pushNarrative(narratives, {
        id: "NARR.App.FirstPartyCredential",
        severity: active.length ? "Critical" : "High",
        priority: "Now",
        title: `Credentials added to ${bySp.size} Microsoft first-party service principal(s)`,
        narrative:
          `${rows.length} client secret(s) / certificate(s) are attached to service principals owned by Microsoft (${[...bySp.keys()].slice(0, 4).join(", ")}${bySp.size > 4 ? ", …" : ""}). ` +
          "First-party apps never need tenant-added credentials: this is the persistence technique documented after tenant compromises — the attacker gets an app-only token with every permission that app already holds, and nothing shows up under App registrations. " +
          (active.length
            ? `${active.length} of them are still valid.`
            : "All of them have expired, but their presence still indicates a past addition that was never investigated."),
        evidence: rows
          .slice(0, 10)
          .map(
            (r) =>
              `${r.SPNDisplayName} (${r.AppId}) — ${r.CredentialType} "${r.CredentialName || "unnamed"}" ${r.Start} → ${r.End}${truthy(r.Expired) ? " (expired)" : ""}`
          )
          .join(" | "),
        remediation:
          "Treat as an incident until proven otherwise: pull 'Add service principal credentials' events from the audit log for these object ids, identify the actor, remove every credential (Remove-MgServicePrincipalPassword / -Key), review the app's consented permissions and sign-ins (27_SPN_SignIns_*), and rotate anything those permissions could reach.",
        relatedFiles: ["04_SPN_Credentials.csv", "27_SPN_SignIns_90d.csv", "25_HighValue_CloudAppEvents_90d.csv"],
        relatedChecks: ["AP.App.FirstPartyCreds"],
      });
    }
  }

  // ── NARR.App.DelegatedBroadGrants — admin-consented delegated scopes for every user ─
  {
    const rows = (ctx.delegatedGrants || []).filter(
      (r) => /Critical|High/.test(r.MaxSeverity || "") && !truthy(r.MicrosoftOwned)
    );
    if (rows.length) {
      const crit = rows.filter((r) => r.MaxSeverity === "Critical");
      pushNarrative(narratives, {
        id: "NARR.App.DelegatedBroadGrants",
        severity: crit.length ? "High" : "Medium",
        title: `${rows.length} third-party app(s) hold tenant-wide delegated grants with write or mailbox scopes`,
        narrative:
          `Admin consent was granted for all users (consentType=AllPrincipals) to ${rows.length} non-Microsoft application(s) with scopes such as ${[...new Set(rows.flatMap((r) => String(r.DangerousScopes).split(" ")))].slice(0, 6).join(", ")}. ` +
          "A delegated grant means any user's session with that app becomes a token for those scopes — phishing one user, or compromising the vendor, yields mailbox, file or directory write on that user's behalf without a new consent prompt." +
          (crit.length
            ? ` ${crit.length} grant(s) include directory- or CA-write scopes that can be converted to tenant-wide privilege.`
            : ""),
        evidence: rows
          .slice(0, 10)
          .map((r) => `${r.ClientDisplayName} → ${r.ResourceDisplayName}: ${r.DangerousScopes}`)
          .join(" | "),
        remediation:
          "For each app: confirm the vendor and business owner, replace AllPrincipals grants with per-user consent or a narrower scope set, remove write scopes the product does not use, and enable the admin consent workflow so future requests are reviewed.",
        relatedFiles: ["04_Delegated_Grants_AllPrincipals.csv", "40_AttackPath_Checklist.csv"],
        relatedChecks: ["AP.App.DelegatedGrants", "AP.Consent.User"],
      });
    }
  }
  ...
  // only includes Admin Portals does not automatically cover that API.
  {
    ...
    if (/partial/i.test(st)) {
      ...
      const tooling = ctx.adminTooling || [];
      const azureCli = tooling.filter((r) =>
        /azure (cli|powershell)|microsoft azure powershell|^azure cli$/i.test(
          String(r.Application || r.App || "")
        )
      );
      const graphCli = tooling.filter((r) =>
        /graph command line|graph explorer/i.test(String(r.Application || r.App || ""))
      );
      const cliSignIns = azureCli.reduce((n, r) => n + (Number(r.SignIns) || 0), 0);
      const cliUsers = [
        ...new Set(
          azureCli.flatMap((r) =>
            String(r.Users || "")
      ...
      const live = cliSignIns > 0;
      pushNarrative(narratives, {
        id: "NARR.CA.AzureMgmt",
        severity: live ? "High" : "Medium",
        priority: live ? "Now" : "Next",
        title: live
          ? `Admins already use Azure CLI / PowerShell (${cliSignIns} sign-ins) without a dedicated MFA policy`
          : "MFA covers the admin browser portals, not Azure CLI / PowerShell",
        impact: live
          ? `A phished password on ${cliUsers.slice(0, 2).join(", ") || "an admin"} yields an ARM token via az / PowerShell — subscriptions, role assignments, runbooks.`
          : "An admin MFA'd in the Azure portal can still obtain an ARM token from the CLI with password-only.",
        narrative:
          "Two different Microsoft first-party apps are involved, and Conditional Access treats them separately. " +
          "Policies that target Microsoft Admin Portals protect the browser admin UIs (Entra, Azure portal, Intune admin). " +
          "They do not cover Azure Service Management (`797f4846-ba00-4fd7-ba43-dac1f8f63013`), which is what `az`, Azure PowerShell, Terraform and ARM REST authenticate against. " +
          (live
            ? `This is not theoretical: ${cliSignIns} Azure CLI / PowerShell sign-in(s) in the window` +
              (cliUsers.length ? ` by ${cliUsers.slice(0, 4).join(", ")}` : "") +
              ". Those tokens are issued for the app this gap leaves open. "
            : "No Azure CLI / PowerShell sign-in was seen in the window — the path is still open the first time someone runs `az login`. ") +
          (graphCli.length
            ? `Graph CLI / Graph Explorer also appear (${graphCli
                .map((r) => `${r.Application || r.App}: ${r.SignIns}`)
                .join("; ")}) — those hit Microsoft Graph (covered by All-apps MFA if you have it), not this ARM gap. `
            : "") +
          "Unrelated to how many devices an admin owns (see NARR.Devices.MultiEndpoint).",
        evidence:
          [
            ...
            live
              ? `adminTooling Azure CLI/PS: ${azureCli
                  .map((r) => `${r.Application}=${r.SignIns}`)
                  .join("; ")} users=${cliUsers.slice(0, 5).join(",")}`
              : "adminTooling Azure CLI/PS: none in window",
          ].join(" | "),
        remediation:
          "1) On the admin MFA policy, add the cloud app Azure Management / Azure Service Management (`797f4846-ba00-4fd7-ba43-dac1f8f63013`) next to Microsoft Admin Portals — or write a dedicated privileged-role policy for that app. 2) Prefer a phishing-resistant authentication strength for those roles. 3) Verify with `az login` / `Connect-AzAccount` from a fresh session that MFA is actually prompted.",
        relatedFiles: [
          "40_AttackPath_Checklist.csv",
          "40_CA_AttackPath_Coverage.csv",
          "02_CA_Audit.csv",
  ...
  // Device join / register MFA (deviceRegistrationPolicy).
  {
    ...
    const joinGroups =
      ...
      [];
    const gaLocal =
      ctx.summary?.deviceJoinGlobalAdminsLocalAdmin === true ||
      (pol.azureADJoin &&
        pol.azureADJoin.localAdmins &&
        pol.azureADJoin.localAdmins.enableGlobalAdmins !== false);
    const regAllLocal = /all registering users/i.test(
      String(ctx.summary?.deviceJoinRegisteringUsersLocalAdmin || "")
    );
    const localNote = gaLocal
      ? ` The same policy still puts Global Administrators in the local Administrators group of every Entra-joined device${regAllLocal ? ", and whoever joins one becomes its local admin" : ""} — a stolen GA token or PRT is then administrator on the fleet (see NARR.Devices.LocalAdmin).`
      : "";
    // Emit when MFA is off (finding), or when MFA is on but Register/Join is All
    // users (scope note). Skip the clean pass: MFA required + join/register restricted.
    if (mfaOff || (mfaRequired && (registerAll || joinAll))) {
      pushNarrative(narratives, {
        ...
        title: mfaOff
          ? "A password is enough to join a rogue device to the tenant"
          : "Device join/register MFA is required (who may join is still broad)",
        impact: mfaOff
          ? "An attacker with one user password enrols a device they control and satisfies device-based CA grants (and, if they are GA, becomes local admin on it)."
          : "Join/register is not a password-only path; remaining question is who is allowed to add devices.",
        narrative: mfaOff
          ...
          : `Device registration MFA is "${mfa}" — join/register is not a password-only path at this control. Remaining scope note: Register is ${registerAll ? "open to all users" : "restricted"}; Join is ${joinAll ? "open to all users" : `limited to ${joinUsers.length || "selected"} user(s)/${joinGroups.length} group(s)`}. This Entra device policy is separate from Conditional Access device-registration user actions.` +
            localNote,
        evidence: `MFA=${mfa || "?"}; Register=${registerAll ? "All users" : regType || "?"}; Join=${joinAll ? "All users" : `${joinType || "selected"} users=${joinUsers.length} groups=${joinGroups.length}`}; quota=${pol.userDeviceQuota ?? ctx.summary?.userDeviceQuota ?? "?"}; gaLocalAdmin=${gaLocal}; registeringUsersLocalAdmin=${ctx.summary?.deviceJoinRegisteringUsersLocalAdmin || "?"}`,
        remediation: mfaOff
          ? "1) Set deviceRegistrationPolicy.multiFactorAuthConfiguration to required. 2) Restrict who can join/register to a named group. 3) Add a CA policy for the 'Register or join devices' user action where licensed. 4) Turn off 'Global administrator is local admin on joined devices' (NARR.Devices.LocalAdmin)."
          : "1) Keep MFA required. 2) Restrict Register from All users if BYOD is not intended. 3) Document who may Join. 4) Review the GA-as-local-admin default (NARR.Devices.LocalAdmin).",
        relatedFiles: [
          "06_device_registration_policy.json",
          "09_Devices_Registered_Only.csv",
          "09_Devices_Per_User_Multi.csv",
        ],
        relatedChecks: ["AP.CA.DeviceReg"],
      });
    }
  }
  ...
  runExtraRules(ctx, narratives, pushNarrative);
  // SecretLifecycle already covers expired / soon-to-expire / long-lived.
  if (narratives.some((n) => n.Id === "NARR.App.SecretLifecycle")) {
    const i = narratives.findIndex((n) => n.Id === "NARR.App.ExpiredSecrets");
    if (i >= 0) narratives.splice(i, 1);
  }
  buildAttackChain(narratives, pushNarrative);
...
function scoreFromNarratives(narratives) {
  ...
  for (const n of narratives) {
    // Synthesis only — already counted via the narratives it names.
    if (n.Id === "NARR.Chain.ShortestPath") continue;
  ...
  // Only a real compensation (All-users MFA + protected registration) softens
  // the gauge; a policy *name* mentioning licences does not.
  const mfa = narratives.find((n) => n.Id === "NARR.MFA.MassGap");
  if (mfa && /compensated=true/i.test(mfa.Evidence || "")) {
    score += 6;
    if (/Critical/i.test(mfa.Severity) === false) score += 2;
  }
  if (mfa && /guestHeavy=true/i.test(mfa.Evidence || "") && !/Critical/i.test(mfa.Severity)) {
    score += 3;
  }
...
function analyzeOutputDir(outDir) {
  ...
  io.saveJson("00_Expert_Findings.json", {
    ...
    score,
    sevCount,
    statusCount: narratives.reduce((acc, n) => {
      const k = n.Status || "Fail";
      acc[k] = (acc[k] || 0) + 1;
      return acc;
    }, {}),
    columns: [
  ...
  if (fs.existsSync(summaryPath)) {
    ...
    summary.usersWithoutMfaGuests = mfaBreak.guest;
