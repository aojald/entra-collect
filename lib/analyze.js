  ...
  // ── NARR.Legacy.Observed+NoCA ─
  {
    ...
    if (legacyFail && legacyRows.length) {
      ...
      pushNarrative(narratives, {
        ...
        evidence:
          "AP.CA.LegacyBlock = Fail / Covered=false" +
          (usageKnown ? "" : "; legacyAuthEvents90d=unknown"),
        remediation:
          "Enforce CA policy blocking legacy authentication for all users." +
          (usageKnown ? "" : " Grant AuditLog.Read.All to measure which accounts still use basic auth before enforcing."),
        relatedFiles: ["40_AttackPath_Checklist.csv", "21_LegacyAuth_Success_90d.csv"],
        relatedChecks: ["AP.CA.LegacyBlock"],
        confidence: usageKnown && !partial ? "High" : "Medium",
      });
    }
  ...
  // ── NARR.MFA.MassGap — context-aware when CA blocks unlicensed / scopes MFA to licenses ─
  {
    ...
    if (n >= 50 || adminsReported >= 1 || humanAdminCount >= 1) {
      ...
      if (humanAdminCount >= 1) {
        ...
        priority = "Now";
        narrative =
          `${n} users have no MFA methods registered, including ${humanAdminCount} human admin(s). ` +
          (compensated
        ...
        narrative =
          ...
          "Track the exclusion-group population and remaining interactive humans — not the raw 07_Users_Without_MFA count.";
        ...
        priority = "Next";
        narrative =
          `${n} users have no MFA methods registered and the tenant does not compensate: ${whyNotCompensated}. ` +
          "Treat as a registration + enforcement gap.";
        if (mfaCa.scoped) {
          narrative +=
        ...
        severity = "Medium";
        priority = "Next";
        narrative = `${n} users have no MFA methods registered and the tenant does not fully compensate (${whyNotCompensated}). Confirm CA actually requires MFA for interactive users who can sign in and protects security-info registration.`;
      }
      pushNarrative(narratives, {
        id: "NARR.MFA.MassGap",
        severity,
        ...
        remediation: compensated
          ? "Do not run a blanket MFA campaign on the raw count. Audit MFA / travel exclusion group membership (make those groups role-assignable), confirm legacy auth is blocked, register MFA for any remaining interactive humans, and treat shared/service objects as sign-in-blocked rather than MFA targets."
          : "Enforce All-users MFA on all cloud apps with a Register-security-info policy that requires a trusted location or compliant device (or a Temporary Access Pass issued by the helpdesk); run the registration campaign; register phishing-resistant methods for human admins first.",
        relatedFiles: [
          "07_Users_Without_MFA.csv",
          "02_CA_Audit.csv",
          "40_CA_Exclusion_Groups.csv",
