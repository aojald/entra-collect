...
(function (root, factory) {
  ...
  function deepDive(id) {
    ...
    if (/^NARR\.(MFA|Identity|Guest|External)\./i.test(x)) return "Users & MFA";
    if (/^NARR\.Chain\./i.test(x)) return "Attack path";
  ...
  function suggestedOwner(n) {
    ...
    if (/attack.?path|graph|privilege.?escalation|^narr\.chain\./.test(blob)) {
      return "Identity / IAM";
    }
  ...
  function buildSheets(D) {
    ...
    const cover = {
      ...
      kpis: [
        ...
        ["  members (interactive humans)", S.usersWithoutMfaHuman ?? ""],
        ["  guests (MFA at home tenant)", S.usersWithoutMfaGuests ?? ""],
    ...
    return [
      ...
      },
      {
        ...
        header: expertHeader,
        rows: expertRows,
        widths: [10, 10, 28, 36, 36, 48, 36, 40, 24, 20, 16, 28, 20, 12],
        wrapCols: [4, 5, 6, 7],
        prioCol: 0,
        sevCol: 1,
        freeze: true,
