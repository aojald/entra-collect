...
/** Room / meeting resource mailboxes — EN / FR / ES. Word-bounded to avoid "salary". */
const ROOM_ACCOUNT_RE =
  /\b(room|meeting|boardroom|conference|huddle|salle|salon|sala|reunion|juntas)\b/i;

/**
 * Shared / departmental mailbox local-part tokens (EN / FR / ES).
 * Distinctive stems only — no short tokens like "hr" or "sala" that hit salary / sarah.
 */
const SHARED_MAILBOX_LOCAL_RE =
  /(no-?reply|noreply|contact|info|communication|event|scan|print|booking|subscription|newsletter|mailing|distribution|shared|internship|compliance|support|helpdesk|payroll|reception|receptionist|frontdesk|front-?desk|welcome|switchboard|accounting|billing|invoic|recruit|mailbox|accueil|compta|facturation|standard|recrut|recepcion|contabilidad|contable|facturacion|nominas?|reclutamiento|rrhh|centralita|bienvenida|atencion|tesoreria|buzon|compras|secretaria)/i;

const SHARED_MAILBOX_DISPLAY_RE =
  /\b(shared(?: mailbox)?|mailbox|distribution list|liste(?: de diffusion)?|training room|conference room|room mailbox|boite partag[eé]|courrier partage|buzon compartido|lista de distribucion|sala de reuniones|correo compartido)\b/i;

/** CA policy names that mean unlicensed / unmanaged population (EN / FR / ES). */
const UNLICENSED_POLICY_RE =
  /unmanaged|unlicensed|sans licence|sin licencia|no licen/i;

function looksLikeUnlicensedPolicyText(s) {
  return UNLICENSED_POLICY_RE.test(foldAccents(s));
}

function looksLikeRoomAccount(upn, displayName) {
  return ROOM_ACCOUNT_RE.test(foldAccents(`${upn || ""} ${displayName || ""}`));
}

function looksLikeServiceAccount(upn, displayName) {
  const local = String(upn || "").split("@")[0] || "";
  const blob = foldAccents(`${upn || ""} ${displayName || ""}`).toLowerCase();
  return (
    /^(svc[-._]|sa[-._]|service[-._]|spn[-._]|sync_|adm[-._]|admin[-._])/i.test(local) ||
    /^(powerbi|noreply|no-reply|saas_|svc)/i.test(local) ||
    /\bservice\b|\bsvc\b|backup|avepoint|sync|provision|automation|break.?glass|power\s*bi|\bservicio\b|\bservico\b/i.test(
      blob
    )
  );
}

function looksLikeSharedMailbox(upn, displayName) {
  const local = foldAccents(String(upn || "").split("@")[0] || "");
  return (
    SHARED_MAILBOX_LOCAL_RE.test(local) ||
    SHARED_MAILBOX_DISPLAY_RE.test(foldAccents(displayName))
  );
}
...
function classifyAccountKind(upn, displayName) {
  ...
  if (looksLikeRoomAccount(upn, displayName)) return "room";
  if (
    looksLikeServiceAccount(upn, displayName) ||
    /^(svc|sa|service|adm|admin)[-_.]|\bservice\b|\bsvc\b/i.test(
      `${upn || ""} ${displayName || ""}`
    )
  ) {
    return "service";
  }
  if (looksLikeSharedMailbox(upn, displayName)) return "shared";
  return "human";
}
